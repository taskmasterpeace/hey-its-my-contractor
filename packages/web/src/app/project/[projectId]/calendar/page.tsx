...
export default function ProjectCalendarPage() {
  ...
  const handleSaveEvent = async (
    eventData: CalendarEvent | Omit<CalendarEvent, "id">
  ) => {
    try {
      const eventWithProject = { ...eventData, project_id: projectId };
      if ("id" in eventData) {
        await updateEventFromHook(eventData.id, eventWithProject);
      } else {
        await createEvent(eventWithProject);
      }
