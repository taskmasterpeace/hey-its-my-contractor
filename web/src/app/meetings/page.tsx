...
import { Meeting, Transcript, MeetingSearchFilters, EnhancedMeetingData, MeetingTag } from '@contractor-platform/types';
...
import { Play, Square, Mic, Users, Clock, FileText, Search, Filter, Calendar, TrendingUp } from 'lucide-react';

export default function MeetingsPage() {
  const [meetings, setMeetings] = useState<EnhancedMeetingData[]>(sampleEnhancedMeetings);
  const [selectedMeeting, setSelectedMeeting] = useState<EnhancedMeetingData | null>(null);
  const [isRecording, setIsRecording] = useState(false);
  const [recordingDuration, setRecordingDuration] = useState(0);
  const [currentView, setCurrentView] = useState<'list' | 'search' | 'transcript'>('list');
  const [searchResults, setSearchResults] = useState(sampleSearchResults);
  const [availableTags, setAvailableTags] = useState<MeetingTag[]>(sampleMeetingTags);
    ...
    setTimeout(() => {
      ...
      if (filters.query) {
        filteredResults = searchResults.filter(result => 
          result.meeting.title.toLowerCase().includes(filters.query!.toLowerCase()) ||
          result.meeting.tags.some(tag => tag.toLowerCase().includes(filters.query!.toLowerCase()))
        );
      }
      
      if (filters.tags && filters.tags.length > 0) {
        filteredResults = filteredResults.filter(result =>
          filters.tags!.some(filterTag => result.meeting.tags.includes(filterTag))
        );
      }
    ...
    <div className="p-6">
      <div className="mb-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 mb-2">Meeting Intelligence</h1>
            <p className="text-gray-600">
              Record meetings, generate transcripts, and extract action items automatically
            </p>
          </div>
            ...
            <div className="flex items-center">
              ...
              <div className="ml-3">
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {meetings.filter(m => m.meeting.recording_url).length}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg border p-4">
            <div className="flex items-center">
              ...
              <div className="ml-3">
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {meetings.reduce((sum, m) => sum + (m.action_items_count || 0), 0)}
                </p>
              </div>
            </div>
          </div>
      ...
      {currentView === 'search' ? (
        <MeetingSearch
          ...
          availableProjects={sampleProjects.map(p => ({
            id: p.id,
            name: p.name
          }))}
        ...
        <div className="space-y-6">
          ...
          {selectedMeeting.transcript ? (
            <EnhancedMeetingTranscript
              ...
              onUpdateTags={(tags) => handleUpdateMeetingTags(selectedMeeting.meeting.id, tags)}
            />
          ) : (
            <div className="bg-white rounded-lg border p-8 text-center">
              ...
              <p className="text-gray-500">
                This meeting doesn't have a transcript yet. Transcripts are automatically generated after meeting recordings are processed.
              </p>
            </div>
          )}
        </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <select className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    ...
                    <option value="change_order">Change Order Discussion</option>
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                  <div className="flex items-start">
                    ...
                    <div className="text-sm">
                      <p className="text-blue-800 font-medium">Consent Required</p>
                      <p className="text-blue-600">
                        All participants will be informed that this meeting is being recorded and transcribed.
                      </p>
                    </div>
                  </div>
                </div>
